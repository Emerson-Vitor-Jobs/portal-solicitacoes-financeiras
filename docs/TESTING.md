...
Não há meta de cobertura (§9.5). Os testes se concentram onde um erro vira dado errado: dinheiro, duplicidade,
...
| Integração | PostgreSQL real (`gex_finance_it`), `TRUNCATE` entre testes, em série | `UNIQUE` com requisições concorrentes de verdade, transições concorrentes, filtros SQL, snapshot da paginação, trigger append-only, dashboard contra o oráculo oficial |
...
O banco de teste é separado porque o teste do dashboard espera exatamente R$ 8.750,49: uma solicitação criada por
outro teste mudaria o número, e o `TRUNCATE` apagaria os dados de quem usa o app. Sem `DATABASE_URL`, a integração
falha em vez de ser pulada, para nunca "passar" sem ter rodado.
...
| #6 | ações simultâneas (concorrência de status) | `requests.integration.test.ts` | aprovar × rejeitar ao mesmo tempo → 1 vence, 1 recebe 409, 1 evento de decisão. O caminho em que o status muda entre a leitura e o `UPDATE` (0 linhas no compare-and-set) é provado de forma determinística no teste de service (`race loser`) |
...
| #13 | solicitação alheia → 404; usuário inexistente = mesma resposta e a mesma verificação argon2 que senha errada | `auth.test.ts`, integração |
