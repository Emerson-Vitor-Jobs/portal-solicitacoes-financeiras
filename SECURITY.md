...
Como o portal protege credenciais, sessões e dados financeiros, o que **não** está coberto neste escopo, e como
reportar um problema. As referências (OWASP, RFCs) e os motivos de cada escolha estão em
...
> Este repositório é um desafio técnico. As credenciais do seed (`data/seed_users.json`, README) são **fictícias e
...
| Revogação real | logout apaga a sessão no servidor; o papel do usuário é relido do banco a cada requisição (não fica "congelado" num token) |
...
### Autorização (§5)
- **No backend, sempre:** os hooks `authenticate` → `requireRole` rodam antes da validação e da busca do recurso. O
  service confere o papel de novo (defesa em profundidade).
- **O REQUESTER só vê o que é dele:** a lista é filtrada no SQL. Abrir a solicitação de outra pessoa responde **404**,
  igual a um id inexistente, pra não revelar quais ids existem (RFC 9110 §15.5.4).
...
- Rate limit no `POST /api/auth/login` com **dois limites independentes**: 5 tentativas por e-mail e 20 por IP, numa
  janela de 15 min (ajustáveis por `LOGIN_RATE_LIMIT_*`). O limite por e-mail vale também para e-mail inexistente, pra
  não revelar contas. Estourou: **429** com `Retry-After`.
- `@fastify/rate-limit` **11.2.0**: as versões anteriores têm bypass por rotação de IPv6 (GHSA-grpc-p53c-r64v).
- **IP real sem spoofing:** a API confia no `X-Forwarded-For` **só** quando a requisição vem do IP fixo do nginx, e o
  nginx sobrescreve o header em vez de anexar o que o cliente mandou.

### Entrada e dados (§2, §6, §7, §11)
- **Validação na borda:** todo corpo, query e parâmetro passa por um schema Zod, e o que é inválido responde 422 com o
  erro por campo. As regras de negócio (dígito do CNPJ, transições, travas de pagamento) ficam no service.
- **O banco como última linha:** `CHECK` de formato e coerência, `UNIQUE (cnpj, nota)` e trigger append-only na
  auditoria. Nenhum caminho (bug, SQL manual) grava um estado impossível.
- **SQL sempre parametrizado** (PgTyped). A busca por fornecedor escapa `%`, `_` e `\`, então um termo digitado nunca
  vira curinga.
- **Dinheiro em inteiro** (centavos, `BIGINT`), sem arredondamento de float.
...
- **Erro de banco reduzido a `{ name, code }`.** O `detail` de um `23505` traria o CNPJ e a nota, e há um teste que
  provoca esse erro e confere que o log não contém nenhum dos dois. O `redact` do pino fica como segunda camada.
- **Erros inesperados** respondem 500 genérico: a mensagem interna nunca chega ao cliente.
...
### Dependências
- Versões **exatas** no `package.json` + `package-lock.json` commitado: o avaliador instala exatamente o que foi testado.
